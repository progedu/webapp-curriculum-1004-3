object BMSearch extends App {
  ...
  def search(text: Seq[Char], pattern: Seq[Char]): Seq[Int] = {
    ...
    while (i < text.length - 1) {
      ...
      val patternLastIndex = pattern.length - 1 // 「パターン」の最後の文字の添え字

      var siMatch = true
      var j = patternLastIndexvar matchChar = '_' //マッチさせた際の「文書」の文字
      var matchPosition = 0 //マッチさせた際の位置（スキップテーブルから取得した値を差し引く値）

      while (j >= 0 && isMatch) {
        if (j > partText.length - 1) { //切り出しテキストが短い際にはfalseに
        isMatch = false
        } else {
          matchChar != pattern(j)) {
            isMatch = false
            matchPosition = (patternLastIndex - j)  // 入り版後ろで不一致なら0　後ろから2番目の時に不一致なら1が代入
          }
        }
        j = j - 1
      }
