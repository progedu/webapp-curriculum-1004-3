...
object BMSearch extends App {
  ...
  println("skiptabale: " + skipTable) //Map(ド　-> 3, ワ -> 2, ン　-> 1, ゴ -> 0)
  ...
  def search(text: Seq[Char], pattern: Seq[Char]): Seq[Int] = {
    ...
    while (i < text.length -1){
      ...
      while (j >= 0 && isMatch){
        if( j > partText.length -1){  //切り出したテキストが短い際には false に
          isMatch = false
        }else {
          matchChar = partText(j)
          if(matchChar != pattern(j)){
            isMatch = false
            matchPosition = (patternLastIndex -j) // 一番後ろで不一致なら０、後ろから２番目の時に不一致なら１が代入
          }
        }
        j = j-1
      }
